...
CREATE TABLE `order_t` (
  ...
  `totalSale` decimal(9,2) DEFAULT NULL,
  `locationID` int DEFAULT NULL,
  `customerID` int DEFAULT NULL,
  PRIMARY KEY (`orderNumber`),
  KEY `order_FK` (`locationID`),
  KEY `order_FK2` (`customerID`),
  CONSTRAINT `order_FK` FOREIGN KEY (`locationID`) REFERENCES `location_t` (`locationID`),
  CONSTRAINT `order_FK2` FOREIGN KEY (`customerID`) REFERENCES `customer_t` (`customerID`)
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_validateOrder` BEFORE INSERT ON `order_t` FOR EACH ROW IF NEW .totalSale <0 THEN
SET NEW .totalSale = 0;
END IF */;;
DELIMITER ;
